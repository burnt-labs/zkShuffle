...
use crate::msg::{
    ExecuteMsg, InstantiateMsg, QueryMsg, VerificationCountResponse,
};
...
fn execute_verify_decrypt_proof(
    ...
    let verified = verify_decrypt_proof(deps.as_ref(), &proof_tuple, &public_inputs, verifier_name)?;
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::VerificationCount {} => {
            to_json_binary(&query_verification_count(deps)?)
        }
    }
}
