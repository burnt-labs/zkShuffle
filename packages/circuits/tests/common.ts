...
describe.skip("Utility circuit unit tests", function () {
  ...
  it("Elliptic curve decompression on baby jubjub curve is correct", async function () {
    ...
    let proveOutput = await snarkjs.groth16.fullProve(
      {
        x: compressedPoint.xArr[0],
        delta: compressedPoint.deltaArr[0],
        s: compressedPoint.selector,
      },
      wasmFile,
      zkeyFile,
    );
  ...
  it("ElGamal Encryption and Decryption are correct", async function () {
    ...
    const encryptionVkey = await snarkjs.zKey.exportVerificationKey(
      new Uint8Array(Buffer.from(readFileSync(encryptionZkeyFile))),
    );
    ...
    const decryptionVkey = await snarkjs.zKey.exportVerificationKey(
      new Uint8Array(Buffer.from(readFileSync(decryptionZkeyFile))),
    );
    ...
    let encryptionProveOutput = await snarkjs.groth16.fullProve(
      {
        ic0: [babyjub.F.toString(ic0[0]), babyjub.F.toString(ic0[1])],
        ic1: [babyjub.F.toString(ic1[0]), babyjub.F.toString(ic1[1])],
    ...
    assert(
      await snarkjs.groth16.verify(
        encryptionVkey,
        encryptionProveOutput.publicSignals,
        encryptionProveOutput.proof,
      ),
      "Off-chain verification failed.",
    );
    ...
    let decryptionProveOutput = await snarkjs.groth16.fullProve(
      {
        c0: [babyjub.F.toString(c0[0]), babyjub.F.toString(c0[1])],
        c1: [babyjub.F.toString(c1[0]), babyjub.F.toString(c1[1])],
        sk: sk,
      },
      decryptionWasmFile,
      decryptionZkeyFile,
    ...
    assert(
      await snarkjs.groth16.verify(
        decryptionVkey,
        decryptionProveOutput.publicSignals,
        decryptionProveOutput.proof,
      ),
      "Off-chain verification failed.",
    );
  });
